...
- Cloud: a separate `public.ranking_preference_events` table, keyed by `(user_id, id)`. Uploads use duplicate-ignore inserts in batches of 50. Reads paginate by the server-assigned, owner-scoped `sync_position` cursor. Each downloaded page and its cursor are checkpointed together; later polls request only newer records and do not rewrite unchanged history. A private transactional owner clock makes the cursor safe against out-of-order commits and client clock skew. RLS and grants allow authenticated users to read/append only their own records; anonymous access and client updates/deletes of history are denied.
...
The additive migration is `supabase/migrations/20260909233047_ranking_preference_events.sql`, aligned with the version applied to the linked Supabase project (`ocpmhsquwsdaauflbygf`). Its primary key also indexes owner lookups and the account foreign key. No displayed-score table or score calculation is altered. The client retains records locally if the table is unavailable. The subsequent `20260910192158_audit_ranking_incremental_sync.sql` migration adds the sync cursor/index and private owner clock; old clients can continue inserting without the cursor column. Complete historical evidence remains both in the server journal and the existing local journal. Local history compaction or a dedicated asynchronous evidence store is still a separate future improvement.
