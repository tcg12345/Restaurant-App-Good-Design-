...
export function GroupPairwise({ ranking, deck, busy, choose, photo, info }: {
  ...
  info?: (place: GroupPlace) => ReactNode;
}) {
  ...
  return <section className="gs-pairwise">
    ...
    <AnimatePresence mode="wait" initial={false}>
      <motion.div className="gs-pair-options" key={`${candidate.id}-${against.id}`} initial={{ opacity: 0, y: reduced ? 0 : 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: reduced ? 1 : .98 }} transition={{ duration: .18 }}>
        {[candidate, against].map(place => <div className="gs-pair-entry" key={place.id}><button disabled={busy} aria-label={`Prefer ${place.name}`} onClick={() => choose({ candidate: candidate.id, against: against.id, preferred: place.id, step: ranking.comparisons ?? 0 })}>
