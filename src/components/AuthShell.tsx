...
const HeroHeading: React.FC<{ line1: string; line2: string }> = ({ line1, line2 }) => (
  <h2 className="font-display tracking-tight leading-[0.92] italic font-semibold">
    <span className="block text-5xl xl:text-6xl text-on-surface">{line1}</span>
    <span className="block text-6xl xl:text-7xl text-primary mt-1">{line2}</span>
  </h2>
);

const PressQuote: React.FC<{ quote: string; source: string }> = ({ quote, source }) => (
  <div className="max-w-md">
    <div className="flex items-start gap-2.5 text-on-surface/65 italic font-serif text-base xl:text-lg leading-snug">
      <span className="text-primary text-4xl leading-[0.5] mt-2 font-serif select-none">&ldquo;</span>
      <span>{quote}</span>
    </div>
    <div className="mt-3 text-[11px] tracking-[0.18em] uppercase text-on-surface/40 font-semibold">
      — {source}
    </div>
  </div>
);
...
const StackedCards: React.FC = () => (
  <div className="relative w-full max-w-[440px] h-[340px] mx-auto">
    {/* Recipe — top-left, smallest, most rotated */}
    <motion.div
      initial={false}
      style={{ rotate: -5 }}
      whileHover={{ rotate: -8, y: -10, x: -4, scale: 1.03 }}
      transition={{ type: 'spring', stiffness: 220, damping: 18 }}
      className="absolute top-0 left-0 xl:left-4 w-[260px] rounded-2xl bg-paper border border-line shadow-md p-4 cursor-pointer z-10"
    >
      <div className="text-[10px] uppercase tracking-[0.14em] text-primary font-bold mb-2">
        Dish · Cacio e pepe
      </div>
      <div className="font-display font-bold text-xl leading-tight text-on-surface mb-1">
        Cacio e pepe
      </div>
      <div className="text-xs text-on-surface/55 mb-2.5">35 min · From your saved</div>
    ...
    <motion.div
      ...
      transition={{ type: 'spring', stiffness: 220, damping: 18 }}
      className="absolute top-[112px] left-[64px] xl:left-[80px] w-[320px] rounded-2xl bg-paper border border-line shadow-md p-4 cursor-pointer z-20"
    >
      <div className="flex items-start gap-3">
        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-secondary to-primary flex items-center justify-center text-white font-bold flex-shrink-0">
          M
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-sm leading-snug text-on-surface">
            <strong className="font-bold">Maya</strong> rated{' '}
            <strong className="font-bold">Don Angie</strong> a perfect 10
          </div>
          <div className="inline-flex items-center gap-1 mt-1.5 px-2 py-0.5 rounded-full bg-primary/10 text-primary text-[10px] font-bold">
            <Sparkles size={10} strokeWidth={2.5} />
            <span>first 10 this year</span>
          </div>
          <div className="text-[11px] text-on-surface/45 mt-1.5">2 hours ago · West Village</div>
        </div>
      </div>
    </motion.div>
    ...
    <motion.div
      ...
      transition={{ type: 'spring', stiffness: 220, damping: 18 }}
      className="absolute bottom-0 right-0 w-[300px] rounded-2xl bg-paper border border-line shadow-lg p-5 cursor-pointer z-30"
    >
      <div className="flex items-start justify-between gap-4 mb-3.5">
        <div>
          <div className="text-[10px] uppercase tracking-[0.14em] text-on-surface/40 font-bold mb-1">Restaurant</div>
          <div className="font-display font-bold text-xl leading-tight">Atomix</div>
          <div className="flex items-center gap-1 mt-1 text-xs text-on-surface/55">
            <MapPin size={11} />
            <span>NoMad · New York</span>
          </div>
        </div>
        <div className="text-right">
          <div className="font-display font-bold text-3xl leading-none text-primary">9.4</div>
  ...
  return (
    <div className="w-full max-w-[420px] mx-auto space-y-3">
      <motion.div
        ...
        className="rounded-2xl bg-paper border border-line shadow-md p-5 cursor-pointer"
      >
        ...
        <div className="grid grid-cols-3 gap-4">
          {stats.map((s) => (
            <div key={s.k}>
              <div className="font-display font-bold text-3xl xl:text-4xl leading-none text-on-surface tabular-nums">
                {s.v}
              </div>
              <div className="text-[10px] uppercase tracking-wider text-on-surface/45 mt-1.5 font-bold leading-tight">
                {s.k}
              </div>
            </div>
          ))}
      ...
      <div className="space-y-1.5 pt-2">
        <div className="text-[10px] uppercase tracking-[0.14em] text-on-surface/45 font-bold px-1 mb-1.5">
          Recently visited
        </div>
        {visits.map((v, i) => (
          <motion.div
            ...
            className="flex items-center gap-3 px-4 py-3 rounded-xl bg-paper/70 hover:bg-paper border border-line/60 hover:border-line transition-colors cursor-pointer"
          >
            <span className={cn('w-1.5 h-1.5 rounded-full', v.dot)} />
            <div className="flex-1 min-w-0">
              <div className="font-display font-bold text-base leading-tight text-on-surface truncate">
                {v.name}
              </div>
              <div className="text-[11px] text-on-surface/55 mt-0.5">{v.loc}</div>
            </div>
            <span className="font-display font-bold text-primary tabular-nums">{v.score.toFixed(1)}</span>
          </motion.div>
        ))}
      </div>
    </div>
...
const HowItWorks: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-[440px] mx-auto space-y-3">
      {steps.map((s) => (
        <motion.div
          key={s.n}
          initial={false}
          whileHover={{ y: -3, scale: 1.01 }}
          transition={{ type: 'spring', stiffness: 220, damping: 20 }}
          className="group flex items-start gap-4 p-4 rounded-2xl bg-paper border border-line hover:border-primary/30 hover:shadow-md transition-[border-color,box-shadow] cursor-pointer"
        >
          <div className="flex flex-col items-center gap-2 flex-shrink-0">
            <span className="text-[10px] tracking-[0.14em] uppercase text-on-surface/40 font-bold font-mono">
              {s.n}
            </span>
            <div className="w-10 h-10 rounded-full bg-primary/10 group-hover:bg-primary/15 flex items-center justify-center text-primary transition-colors">
              <s.icon size={18} strokeWidth={2} />
            </div>
          </div>
          <div className="flex-1 min-w-0">
            <div className="font-display font-bold text-lg leading-tight text-on-surface">{s.title}</div>
            <div className="text-sm text-on-surface/55 mt-1 leading-relaxed">{s.desc}</div>
          </div>
        </motion.div>
      ))}
    </div>
...
const ProfilePreview: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-[400px] mx-auto space-y-3">
      ...
      >
        <div className="h-20 bg-gradient-to-br from-primary/30 via-accent/40 to-secondary/30 relative">
        ...
        <div className="px-5 pb-5 -mt-10 relative">
          <div className="w-20 h-20 rounded-full border-4 border-paper shadow-md flex items-center justify-center bg-gradient-to-br from-primary to-secondary text-white font-serif italic text-3xl">
            T
          </div>
          <div className="mt-3">
            <div className="font-display font-bold text-2xl leading-tight text-on-surface">Your name</div>
            <div className="text-sm text-on-surface/55">@your_handle</div>
          </div>
          <div className="flex items-center gap-1 mt-2 text-xs text-on-surface/55">
            <MapPin size={12} />
            <span>Your home city</span>
          </div>
          <div className="flex items-center gap-5 mt-4">
            <div>
              <div className="font-display font-bold text-lg leading-none tabular-nums">0</div>
              <div className="text-[10px] uppercase tracking-wider text-on-surface/45 mt-1 font-bold">places</div>
            </div>
            <div>
              <div className="font-display font-bold text-lg leading-none tabular-nums">0</div>
              <div className="text-[10px] uppercase tracking-wider text-on-surface/45 mt-1 font-bold">followers</div>
            </div>
            <div>
              <div className="font-display font-bold text-lg leading-none tabular-nums">0</div>
              <div className="text-[10px] uppercase tracking-wider text-on-surface/45 mt-1 font-bold">following</div>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="space-y-1.5">
        ...
        {lists.map((l) => (
          <motion.div
            ...
            whileHover={{ x: 4 }}
            transition={{ type: 'spring', stiffness: 300, damping: 25 }}
            className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-paper/70 hover:bg-paper border border-line/60 hover:border-line transition-colors cursor-pointer"
          >
            <Heart size={14} className="text-primary/70" />
            <span className="font-display font-bold text-sm flex-1 text-on-surface">{l.name}</span>
            <span className="text-[11px] text-on-surface/45 tabular-nums">{l.count} places</span>
          </motion.div>
        ))}
      </div>
    </div>
    ...
    <div className={cn('relative h-full overflow-hidden text-on-surface', p.bg)}>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="relative h-full flex flex-col p-10 xl:p-14"
        >
          {/* Brand bar */}
          ...
          <div className="max-w-md mb-6">
            ...
            <p className="text-base xl:text-lg text-on-surface/55 font-light leading-relaxed mt-5">
              {p.subtitle}
            </p>
          </div>

          {/* Visual composition — flex-1 so cards center vertically */}
