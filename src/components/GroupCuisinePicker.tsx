  ...
  return <>
    ...
    <dialog ref={dialog} className="gs-cuisine-dialog" aria-labelledby="gs-cuisine-title" onCancel={() => setOpen(false)} onClick={e => { if (e.target === dialog.current) setOpen(false); }}>
      <section className="gs-cuisine-sheet">
        ...
        <div data-search-field className="gs-cuisine-search"><Search size={17} /><input data-search-input="embedded" aria-label="Search cuisines" placeholder="Search cuisines" value={query} onChange={e => setQuery(e.target.value)} />{query && <button aria-label="Clear cuisine search" onClick={() => setQuery('')}><X size={15} /></button>}</div>
