...
import { X, Plus, Check, Camera, Trash2, ChevronLeft, ChevronRight, DollarSign, CalendarDays, Tag, StickyNote, Image, ListPlus } from 'lucide-react';
        ...
        >
          ...
          >
            <AnimatePresence mode="wait">
                  ...
                  <div className="px-5 pt-4 sm:pt-5 pb-2 flex items-center justify-between flex-shrink-0">
                    <div className="min-w-0">
                      <h2 className="font-serif font-bold text-lg truncate">
                        {existing ? 'Update Rating' : 'Rate Restaurant'}
                      </h2>
                    ...
                    <div className="flex flex-col items-center pt-4 sm:pt-6">
                      <div className={cn("relative w-32 h-32 sm:w-36 sm:h-36 rounded-full flex items-center justify-center mb-4 bg-gradient-to-b ring-4", scoreBg, scoreRing)}>
                        <div className="text-center">
                          <div className={cn("text-5xl sm:text-5xl font-serif font-bold tabular-nums transition-colors duration-300", scoreColor)}>
                            {score.toFixed(1)}
                          </div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        <button onClick={() => setPage('notes')}
                          className={cn("flex flex-col items-center gap-1.5 p-3 rounded-2xl border transition-all",
                            hasNotes ? "bg-primary/5 border-primary/20" : "bg-white border-on-surface/8 hover:border-on-surface/15"
                          )}>
                        ...
                        {/* Price — inline popup */}
                        <button onClick={() => setPage('price')}
                          className={cn("flex flex-col items-center gap-1.5 p-3 rounded-2xl border transition-all",
                            hasPrice ? "bg-primary/5 border-primary/20" : "bg-white border-on-surface/8 hover:border-on-surface/15"
                          )}>
                        ...
                        <button onClick={() => setPage('tags')}
                          className={cn("flex flex-col items-center gap-1.5 p-3 rounded-2xl border transition-all",
                            hasTags ? "bg-primary/5 border-primary/20" : "bg-white border-on-surface/8 hover:border-on-surface/15"
                          )}>
                        ...
                        {/* Photos */}
                        <button onClick={() => { setPage('photos'); }}
                          className={cn("flex flex-col items-center gap-1.5 p-3 rounded-2xl border transition-all",
                            hasPhotos ? "bg-primary/5 border-primary/20" : "bg-white border-on-surface/8 hover:border-on-surface/15"
                          )}>
                        ...
                        {/* Lists */}
                        <button onClick={() => setPage('lists')}
                          className={cn("flex flex-col items-center gap-1.5 p-3 rounded-2xl border transition-all",
                            hasLists ? "bg-primary/5 border-primary/20" : "bg-white border-on-surface/8 hover:border-on-surface/15"
                          )}>
              ...
              {/* ═══════════ NOTES PAGE ═══════════ */}
              {page === 'notes' && (
                <motion.div key="notes" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 30, stiffness: 300 }}
                  className="flex flex-col h-full">
                  ...
                  <div className="flex-1 overflow-y-auto px-5 py-5">
                    <textarea
                      ...
                      placeholder="What did you enjoy? Any favorite dishes, standout moments, or things to remember?"
                      rows={8}
                      autoFocus
                  ...
                  <div className="px-5 py-4 flex-shrink-0 border-t border-on-surface/6 bg-surface">
                    <button onClick={() => setPage('main')} className="w-full py-3 bg-primary text-white rounded-2xl font-semibold text-sm active:scale-[0.98] transition-transform">
                      {hasNotes ? 'Update Notes' : 'Save Notes'}
                    </button>
                  </div>
                </motion.div>
              )}

              {/* ═══════════ PRICE PAGE (compact popup style) ═══════════ */}
              {page === 'price' && (
                <motion.div key="price" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 30, stiffness: 300 }}
                  className="flex flex-col h-full">
              ...
              {/* ═══════════ DATE PAGE (compact popup style) ═══════════ */}
              {page === 'date' && (
                <motion.div key="date" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 30, stiffness: 300 }}
                  className="flex flex-col h-full">
              ...
              {/* ═══════════ TAGS PAGE ═══════════ */}
              {page === 'tags' && (
                <motion.div key="tags" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 30, stiffness: 300 }}
                  className="flex flex-col h-full">
                  ...
                  <div className="px-5 py-4 flex-shrink-0 border-t border-on-surface/6 bg-surface">
                    <button onClick={() => setPage('main')} className="w-full py-3 bg-primary text-white rounded-2xl font-semibold text-sm active:scale-[0.98] transition-transform">
                      {hasTags ? `Done (${selectedTags.length})` : 'Done'}
                    </button>
                  </div>
                </motion.div>
              )}
              ...
              {/* ═══════════ PHOTOS PAGE ═══════════ */}
              {page === 'photos' && (
                <motion.div key="photos" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 30, stiffness: 300 }}
                  className="flex flex-col h-full">
                  ...
                  <div className="flex-1 overflow-y-auto px-5 py-5">
                    ...
                    {photos.length === 0 ? (
                      <button onClick={() => fileInputRef.current?.click()}
                        ...
                        <span className="text-[11px] opacity-60">Share your experience</span>
                      </button>
                    ) : (
                      <>
                  ...
                  <div className="px-5 py-4 flex-shrink-0 border-t border-on-surface/6 bg-surface">
                    <button onClick={() => setPage('main')} className="w-full py-3 bg-primary text-white rounded-2xl font-semibold text-sm active:scale-[0.98] transition-transform">
                      {hasPhotos ? `Done (${photos.length})` : 'Done'}
                    </button>
                  </div>
                </motion.div>
              )}
              ...
              {/* ═══════════ LISTS PAGE ═══════════ */}
              {page === 'lists' && (
                <motion.div key="lists" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 30, stiffness: 300 }}
                  className="flex flex-col h-full">
                  ...
                  <div className="px-5 py-4 flex-shrink-0 border-t border-on-surface/6 bg-surface">
                    <button onClick={() => setPage('main')} className="w-full py-3 bg-primary text-white rounded-2xl font-semibold text-sm active:scale-[0.98] transition-transform">
                      {hasLists ? `Done (${selectedListIds.length})` : 'Done'}
                    </button>
                  </div>
                </motion.div>
              )}
