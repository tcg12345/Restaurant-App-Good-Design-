...
export const HomeExperience: React.FC<Props> = ({ active, name, city, highlights, onHighlightLink, onHighlightSeen, onHighlightOpen, onLocation, onSearch, onAction, header, feed, guides, feedFilters, onPageChange }) => {
  ...
  useEffect(() => {
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      if (page === 'home' && scrollTop() > 2) return;
    ...
    <motion.div className="home-track" initial={false} animate={{ y: page === 'feed' ? '-100%' : '0%' }}
      ...
      <section ref={homeScroll} className="home-launch" aria-label="Home" aria-hidden={page !== 'home' || !active} inert={page !== 'home' || !active}>
        <header className="home-chrome home-topbar">
          <button className="home-location" onClick={onLocation} aria-label={`Change dining location: ${city}`}><MapPin size={16} /><span>{city}</span><ChevronDown size={13} /></button>
          {header}
        </header>
        <div className="home-launch-content">
          <div className="home-welcome">
            <p>GoodEats · A little inspiration, every day</p>
            <h1>{name?.trim() ? `What sounds good, ${name.trim()}?` : 'What sounds good?'}</h1>
          </div>
          ...
          <button ref={exploreButton} className="home-explore" onClick={() => go('feed', true)}><strong>Explore feed</strong><span className="home-explore-arrow" aria-hidden="true"><ArrowDown size={16} /></span></button>
        </div>
      </section>
