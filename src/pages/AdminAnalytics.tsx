...
import type { ExplorationVisitor } from '../lib/analytics-exploration';
...
type Tab = 'Overview' | 'Exploration' | 'Pages' | 'Features' | 'Restaurants' | 'APIs' | 'Users' | 'Search' | 'Outcomes';
const tabs: Tab[] = ['Overview','Exploration','Pages','Features','Restaurants','APIs','Users','Search','Outcomes'];
...
const sourceLabels: Record<string, string> = { google_places: 'Google Places', own_data: 'Our data', mixed: 'Mixed data', unknown: 'Unknown / older tracking' };
function RestaurantSources({ row }: { row: Row }) {
 const sources: Row[] = row.sources || [];
 if (!sources.length) return <span>Unknown / older tracking</span>;
 return <details className="analytics-sources"><summary>{sources.map(s => sourceLabels[s.data_source] || 'Unknown').join(' + ')}</summary>
  <Table rows={sources} columns={[col('data_source','Data source',s=>sourceLabels[s.data_source]||'Unknown'),col('searches','Search selections'),col('returned','Returned'),col('seen','Seen'),col('opens','Detail visits'),col('saves','Saves'),col('unsaves','Removals'),col('ratings','Ratings'),col('shares','Shares'),col('outbound','Outbound taps')]} />
 </details>;
}
...
export function AnalyticsDashboard({ isAdmin, adminChecked, authLoading }: { isAdmin: boolean; adminChecked: boolean | 'unknown'; authLoading: boolean }) {
 ...
 const [explorationVisitor,setExplorationVisitor]=useState<ExplorationVisitor|null>(null);
 ...
 async function loadMore(){
  ...
  try {
   ...
   if(requested!==actorRef.current)return;
   if(error)setTimelineError(error.message);else{setTimeline(t=>[...t,...(data||[])]);setMore(data?.length===100);}
  } catch { if(requested===actorRef.current)setTimelineError('Connection interrupted. Try loading older activity again.'); }
  finally { if(requested===actorRef.current)setTimelineBusy(false); }
 }
 ...
 return <div className="analytics-page" data-analytics-private>
  ...
  {tab!=='Exploration'&&error&&<div role="alert" className="analytics-notice analytics-error"><strong>Analytics could not load.</strong><p>{error}</p><p>Apply the owner_analytics migration to this app’s Supabase project and confirm your account is in app_admins.</p></div>}
  ...
  {tab==='Exploration'&&<AnalyticsExploration days={days} platform={platform} refresh={refresh} visitor={explorationVisitor} onVisitorChange={setExplorationVisitor}/>}
  {tab!=='Exploration'&&!report&&!error&&<div className="analytics-empty">{busy?'Loading your analytics…':'No report loaded.'}</div>}
  {tab!=='Exploration'&&report&&<>
   {tab==='Overview'?<>
    ...
    {tab==='Restaurants'&&<Table rows={rows} columns={[col('name','Restaurant',r=><Link to={`/restaurant/${encodeURIComponent(r.restaurant_id)}`}>{r.name||r.restaurant_id} <ArrowUpRight size={12}/></Link>),col('sources','Data source by action',r=><RestaurantSources row={r}/>),col('searches','Search selections'),col('returned','Returned'),col('seen','Seen'),col('opens','Detail visits'),col('visitors','Visitors'),col('saves','Saves'),col('unsaves','Removals'),col('ratings','Ratings'),col('shares','Shares'),col('outbound','Outbound taps'),col('api_calls','API calls'),col('cost','Est. cost',r=>money(r.cost))]}/>}
