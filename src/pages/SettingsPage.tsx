...
{phoneSent && <button className="settings-text-button" disabled={phoneBusy} onClick={() => { setPhoneSent(false); setPhoneCode(''); setAccountMsg(''); }}>Use a different number</button>}</div>}
      ...
      {page === 'privacy' && <>
        <SafetySettings />
        {group('AI data sharing', <><p className="settings-explainer">AI features share the content you choose and relevant food preferences with Anthropic and OpenAI only after you allow it.</p><Row icon={<Shield size={19} />} title="Turn off AI sharing" sub="Ask again before the next AI request" onPress={() => { if (user) { revokeAiConsent(user.id); showToast('AI sharing is off. We’ll ask before your next AI request.'); } }} /></>)}
