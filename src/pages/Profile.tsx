...
import { Settings, LogOut, X, User, AtSign, Check, ChevronRight, Smartphone, Lock, Mail, Trash2, ArrowLeft, AlertTriangle } from 'lucide-react';
...
type SettingsPage = 'main' | 'profile' | 'account';
...
export const Profile: React.FC = () => {
  ...
  const handleSaveProfile = async () => {
    ...
    if (result.success) { setEditSuccess(true); await refreshProfile(); }
      ...
      <main className="px-3">
        <section className="flex flex-col items-center mb-8">
          ...
          <button onClick={openSettings}
            className="flex items-center gap-1.5 px-5 py-2.5 rounded-full bg-on-surface/5 border border-on-surface/10 text-xs font-semibold text-on-surface/60 hover:bg-on-surface/8 transition-colors">
        ...
        {user?.email && (
          <p className="text-center text-xs text-on-surface/30 mb-4">{user.email}</p>
        )}
      </main>
                ...
                {settingsPage === 'main' && (
                  <motion.div key="main" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, x: -20 }}
                    ...
                    <div className="flex-1 overflow-y-auto px-5 py-4 space-y-1">
                      <button onClick={() => { setSettingsPage('profile'); setEditError(''); setEditSuccess(false); }}
                        ...
                        <div className="flex-1"><p className="text-sm font-medium">Edit Profile</p><p className="text-[11px] text-on-surface/35">Name, username</p></div>
                      ...
                      <div className="border-t border-on-surface/6 pt-4">
                        ...
                        {deleteStep === 2 && (
                          <div className="bg-red-100 border border-red-300 rounded-xl p-3 space-y-2">
                            ...
                            <div className="flex gap-2">
                              <button onClick={() => setDeleteStep(0)} className="flex-1 py-2 border border-on-surface/15 rounded-lg text-xs font-semibold text-on-surface/50">Cancel</button>
                              <button onClick={handleDeleteAccount} className="flex-1 py-2 bg-red-700 text-white rounded-lg text-xs font-semibold">Delete Forever</button>
                            </div>
                          </div>
                        )}
                      </div>
