-- Private authoritative state. Only the authenticated Edge handler can mutate it.
create schema if not exists private;
revoke all on schema private from public, anon;
create table private.group_rooms (
 ...
 state jsonb not null, model jsonb not null default '{}'
);
create index group_rooms_host_created on private.group_rooms(host, created_at desc);
create table private.group_attempts (user_id uuid primary key references auth.users(id) on delete cascade, window_start timestamptz not null default now(), count int not null default 0);
create table public.group_room_events (id uuid primary key references private.group_rooms(id) on delete cascade, revision bigint not null default 0);
alter table public.group_room_events enable row level security;
revoke all on public.group_room_events from anon, authenticated;
grant select on public.group_room_events to authenticated;
create or replace function private.can_read_group(room_id uuid) returns boolean language sql stable security definer set search_path='' as $$
 select exists(select 1 from private.group_rooms where id=room_id and expires_at>now() and state->'members' ? auth.uid()::text);
$$;
revoke all on function private.can_read_group(uuid) from public;
grant usage on schema private to authenticated;
grant execute on function private.can_read_group(uuid) to authenticated;
create policy group_events_member on public.group_room_events for select to authenticated using (private.can_read_group(id));
alter publication supabase_realtime add table public.group_room_events;

-- Actor is supplied only by an Edge handler after getUser verifies the JWT.
create or replace function public.group_room_action(actor uuid, action text, payload jsonb default '{}') returns jsonb
...
begin
 ...
 if action='list' then
  return coalesce((select jsonb_agg(jsonb_build_object('id',id,'code',code,'location',state->'location','status',state->'status')) from private.group_rooms where expires_at>now() and state->'members' ? actor::text),'[]');
 end if;
...
-- Replace helper use with an inline count to keep the function dependency-free.

revoke all on function public.group_room_action(uuid,text,jsonb) from public, anon, authenticated;
grant usage on schema private to service_role;
grant all on private.group_rooms, private.group_attempts, public.group_room_events to service_role;
